*Drawn in large part from:* [Best Practices for Scientific Computing (Wilson etal 2014)](http://journals.plos.org/plosbiology/article?id=10.1371/journal.pbio.1001745)
...
* No Suspense! (This good for mystery novel, BAD for coding and code documentation.)
...
The GCAM Data System (gcamdata) project is one example of both of these approaches. The overall design is highly modular (e.g., split in small chunks that produce specific outputs). Also, each code chunk follows a similar pattern of code blocks (e.g. purpose documented at the top, files read in, then processing, and output at the end), with a certain level of required documentation, including units, required.
...
is ***not helpful***, and does not help anyone understand what is going on. 
...
for ocean_basins = 1, NUM_OCEAN_BASINS { 
```

tells the reader much more about what is going on.

### Use Consistent Formatting Conventions

Each project should file a consistent set of coding guidelines, which lay out formatting conventions for variables, functions, constants, documentation, indentation, and so on. (Note, variables and functions should have a different format!)

> Borrow a coding style guide from an existing project and, if needed, adapt that to your own project needs.

One example of a commonly used style, which is to denote constants with capital letters. For example:

`NUM_OCEAN_BASINS = 5`

With this as a standard, the user is reminded whenever this quantity is used that it is a constant and not a variable that can be changed. (e.g. reducing the number of pieces of information that need to be in working memory.) Also, even if this isn't enforced by the code (for example in R), the user is less likely to write:
...
* Your comment lines will more likely be than your code! So make them useful and readable.
...
* This also will help avoid errors and problems buried in the middle of a complex calculation. (e.g. better to "show your work" and break into smaller pieces.)
...
Even if you don't actually use the documentation generator, the documentation formats can still be useful. (e.g. in GCAM use of Doxygen style comments have resulted in consistent and more fully documented code even though we have rarely actually used the system to generate separate documentation).
...
is much clearer than a 5-page code chunk that does everything in one block.
...
Once you've diagramed your code, talk it over with someone else. It doesn't have to be someone in your team (its often better if it is not someone on your team).
...
* It means that if this number were to change in the future (such as the number of ocean basins in the example), someone would have to search through the code to change all instances of this. Instead this constant should be defined in a way that is available to all the code that needs this information.  
* It also means that there is a number with no context, making the code harder to understand.
...
* If the function is well-named this also means that the functionality can be remembered (and referenced) as one single chunk (e.g., less stress on working memory)
...
Sometimes a loop is necessary, but in most higher level languages using a for loop reduces or eliminates the ability of the complier or interpreter to optimize or parallelize the operation. 

If a loop is required avoid hard coding limits, such as was done in our [bad code example](#bp-bad-code-example) above. Many languages allow you to identify the number of elements in a multi-dimensional variable, use that capability. In C++ use iterators.

In a language such as R, functions such as apply and lapply are more efficient and powerful means of achieving what you would otherwise do in loop. This requires you to break your operations into discrete functions, but after reading this document you now are going to do this anyway, right?
...
Assertions are checks a chunk of code is getting values that are valid for the operations that are being performed. As you write code, ask yourself *under what conditions will this fail*, and then write assertions to check for those conditions.

**For most scientific codes, if a necessary condition is not met the code should error and stop. Otherwise you risk getting an invalid result.**
...
For example (revisiting the example from section 3), if working in many languages you can declare a variable as a constant who's value cannot be changed:
...
> Assertions can make up a sizeable fraction of the code in well-
written applications, just as tools for calibrating scientific instruments can make up a sizeable fraction of the equipment in a lab (Wilson et al 2014).
...
In many cases for physical quantities data is additive, so as processing proceeds you can check that disaggreated data add up to totals.

* This can be particularly useful when reading and processing input data. You can sometimes check that your sum adds up to the "total" line in the original data.
...
### Pair programing
>in which two developers sit together while writing code. One (the driver) actually writes the code; the other (the navigator) provides real- time feedback and is free to track larger issues of design and consistency. Several studies have found that pair programming improves productivity (Wilson et al. 2014)

Even if done only for a short time, or when tackling a tricky program, this helps you "get out of your own head" and the overall effort can benefit from the interaction.
...
The major advantage of version control is that it facilitates collaboration. It makes it feasible for multiple people to work on a project at one time. Many version control server systems (such as gitHub, JIRA) facilitate code reviews, which makes it easy for others to comment and for you to respond to those comments.

So use version control. Once you have that set-up your repository can easily be put on-line. (The on-line repository also serves as an off-site backup, but you already have automatic, off-site backup, right? If not set that up *right now*.)
